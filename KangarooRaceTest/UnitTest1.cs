...
namespace KangarooRaceTest
{
    ...
    public class UnitTest1
    {
        ...
        public void TestWinnerOutcome()
        {
            ...
            Joban = pFactory.getPunter("Joban", null, null);
            Joban.Cash = BettingAmount;
            Joban.PlaceBet((int)BettingAmount, KangarooNumber);
            ...
            if (Joban.bet.KangarooNum == win)
            {
                Assert.AreEqual(expectedWin, Joban.Cash, "Account not credited correctly");
            }
            if (Joban.bet.KangarooNum != win)
            {
                Assert.AreEqual(expectedLose, Joban.Cash, "Account not debited correctly");
